...
form.addEventListener('submit', e => {
    ...
    .then(json => {
        if (json.ok) {
            form.reset();
            showToast('RSVP disimpan ke Google Sheet!');
            document.getElementById('success-msg').textContent = 'Terima kasih! RSVP disimpan. 🎉';
            document.getElementById('success-msg').style.display = 'block';
            createConfetti();
            setTimeout(() => { if (typeof fetchRemoteGuestbook === 'function') fetchRemoteGuestbook(); }, 500);
        } else {
            alert('Ralat: ' + (json.error || 'Server error'));
        }
...
// Fetch live guestbook from Google Sheet via Apps Script
